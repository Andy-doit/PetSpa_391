...
import { ShopAccountInfor, updateProfileShopInput } from '@/models/shopModel';
import { patchUpdateProfileShop, ShopInforPagination } from '@/lib/redux/slice/shopSlice';
...
const ProfileAccount: React.FC = () => {
    ...
    const [items, setItems] = useState<ShopAccountInfor | null>(null);
    ...
    useEffect(() => {
        const fetchUserInformation = async () => {
            if (userId) {
                try {
                    const response = await dispatch(ShopInforPagination());
    ...
    const handleUpdate = async (values: updateProfileShopInput, { setSubmitting }: FormikHelpers<updateProfileShopInput>) => {
        try {
            if (userId) {
                const updateData: updateProfileShopInput = {
                    id: userId,
                ...
                await dispatch(patchUpdateProfileShop({ profileData: updateData })).unwrap();
